/**
 * Deterministic Synthetic Report & Evidence Fixtures
 * Trust, Safety & Digital Security Project
 *
 * Contains:
 * - 2 VERIFIED claims (with explicit supporting evidence)
 * - 2 CONTRADICTED claims (with explicit contradicting evidence)
 * - 2 INSUFFICIENT_EVIDENCE claims (where evidence is missing or indecisive)
 *
 * All evidence preserves full provenance:
 * - evidence ID
 * - source / reference
 * - relevant excerpt or value
...
export const RAW_SYNTHETIC_REPORT: SyntheticReport = {
  ...
  description:
    'Independent digital security audit evaluating public metrics, transport encryption integrity, and endpoint exposure for public open-source project SuperPrompt CLI.',
  claims: [
    {
      id: 'claim_github_stars',
      statement:
        'The open-source repository "alexdev/superprompt-cli" has reached or exceeded the 1,000 public GitHub stars growth threshold.',
      ...
      status: 'INSUFFICIENT_EVIDENCE', // Initial state before processing
    ...
    },
    {
      ...
      statement:
        'The production documentation website enforces Strict-Transport-Security (HSTS) with a minimum duration of 31,536,000 seconds (1 year).',
    ...
    },
    {
      ...
      statement:
        'All incoming unencrypted HTTP requests to port 80 are strictly redirected with an HTTP 301/308 redirect to HTTPS.',
      category: 'Transport Security',
      ruleType: 'STRING_INCLUSION',
    ...
    },
    {
      ...
      statement:
        'The public web server does not expose internal Git version control directories (.git/HEAD) on publicly accessible endpoints.',
    ...
    },
    {
      ...
      statement:
        'All DNS Certification Authority Authorization (CAA) records for "superprompt-cli.dev" strictly restrict certificate issuance to Let\'s Encrypt.',
    ...
    },
    {
      ...
      statement:
        'Internal persistence volumes and database storage partitions are encrypted at rest using AES-256 with customer-managed keys.',
